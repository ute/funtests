...
#'splot method for class fdsample
#'
#'Plots an object of class \code{fddsample}. The function \code{splot.fdsample}
#'is an alias for \code{\link{plot.fdsample}}
#'The individual functions contained in the \code{\link{fdsample}} objects
#'are plotted.
#'
#'@param x the fdsample to be plotted
#'@param includy optional numeric vector containing values that are to be included in the
...
#'in package \code{plottools.}
#'
#'Plot parameter arguments can be given as \code{"\link{simplist}"}s or separately.
#'
#'@S3method splot fdsample
#'@method splot fdsample
# @author Ute Hahn,  \email{ute@@imf.au.dk}
#'@seealso \code{\link{alphacol}} on defining transparent colors, and
#'\code{\link{splot}} for the evaluation of plot parameters.
#'@examples
...
#'# using a predefined list of options
#'blau <- simplist(col = "blue", alpha = 0.4)
#'splot(fuda, blau, includy = -2)
#'# add mean
#'splot(mean(fuda), col = "blue", lwd = 2, add = TRUE)

splot.fdsample <- function(x, ..., includy = NULL) {
  plot.fdsample(x, ..., includy = includy)
}